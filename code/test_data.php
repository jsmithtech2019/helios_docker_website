...
while($row = mysqli_fetch_array($customer))
{
    ...
    echo "<h2 align='left'>" . $row['name'] . "</h2>";
    ...
    $truck = mysqli_query($con,"SELECT test1_current, test1_result, test2_current, test2_result, test3_current, test3_result, test4_current, test4_result FROM TRUCK_TEST_DATA WHERE (cust_phone='{$row['phone']}') OR (cust_email='{$row['email']}')");
        ...
        foreach ($truckrow as $key => $value) {
            if (strpos($key, 'result')){
                if ($value > 0){
                    ...
                    echo "<td align='center' style='font-size: 14px;' bgcolor='#CD5C5C'>Fail</td>";
                }
